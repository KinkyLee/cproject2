...
02.   ===================================== 
03.   These programs are Copyright (c) University of British Columbia. All rights reserved. 
04.   They may be freely redistributed in their entirety provided that this copyright 
05.   notice is not removed. THEY MAY NOT BE SOLD FOR PROFIT OR INCORPORATED IN 
06.   COMMERCIAL PROGRAMS WITHOUT THE WRITTEN PERMISSION OF THE COPYRIGHT HOLDER. 
07.   Each program is provided as is, without any express or implied warranty, 
08.   without even the warranty of fitness for a particular purpose. 
09. 
10.   ========================================================= 
...
static char *banner="COPYRIGHT HEWLETT-PACKARD COMPANY, 1995-1999.\n";  
...
{  
    ...
    {  
        ...
        {
        	fprintf(stderr,"Input file is truncated %d",ii_count);

        }

 /*         int linebyte=512*10/8; 
		  int k=0;
		  unsigned char *linebuf;
		  linebuf=malloc(linebyte);

		  fread(linebuf, 1, linebyte, infile);
		  
		  for(i=0;i<640;i=i+5)
		  {
			  line[k]=linebuf[i]*4+(linebuf[i+1]>>6);
			  line[k+1]=(linebuf[i+1]&0x3F)*16+linebuf[i+2]/16;
			  line[k+2]=(linebuf[i+2]&0x0F)*64+linebuf[i+3]/4;
			  line[k+3]=(linebuf[i+3]&0x03)*64*4+linebuf[i+4];
			  k=k+4;
...
void initialize(int argc, char *argv[],int near,int imgcol,int imgrow)
{  
...
//        }
                    ...
                    infilename = c_infilename[components++] = "temp_16.raw";
                    gotinf = 1;
                    outfilename = "temp.raw";
                    gotoutf = 1;
...
#define LESS_CONTEXTS 1  
    ...
    if ( verbose>1 )  
        printf("Number of contexts (non-run): %d regular + %d EOR = %d\n",CONTEXTS-LESS_CONTEXTS,EOR_CONTEXTS,TOT_CONTEXTS-LESS_CONTEXTS);  
      
    /* Read image headers*/  
    if (multi==0) {  
        if ( read_header_6(in, &columns, &rows, &alpha0, &components,imgcol,imgrow) != 0 )
...
/* check that alpha is a power of 2 */  
    ...
    if ( verbose ) {  
        if (!multi)  
            ...
            printf("Input  files: ");  
            for (i=0;i<components;i++)  
                printf(" %s ",c_infilename[i]);  
            printf("\nOutput file: %s\n",outfilename);  
        }  
  
        if (!multi)  
            printf("Image: cols=%d rows=%d alpha=%d comp=%d mode=%d (%s)",  
                columns, rows, alpha, components,   
                color_mode, color_mode_string);  
        else {  
            printf("Image: cols=");  
            for (i=0;i<components;i++)  
                printf(" %d",c_columns[i]);  
            printf(" rows=");  
            for (i=0;i<components;i++)  
                printf(" %d",c_rows[i]);  
            printf(" alpha=%d comp=%d mode=%d (%s)",  
    ...
    if (lossy==TRUE) {  
        ...
        negNEAR = -NEAR;  
        if ( verbose )  
            printf("Near-lossless mode: NEAR = %d  beta = %d  qbeta = %d\n",NEAR,beta,qbeta);  
    }  
...
int encode(int argc, char *argv[],int near,int imgcol,int imgrow,int imgdepth ){
//int encode(FILE *f_in,FILE *f_out,int near,int imgcol,int imgrow,int imgdepth ){
...
//         pos0, pos1;
    ...
    application_header = all_header = 0;  
    			  ...
    			  //  int argc=3;
                  //  char argv[3][20]={"-itemp_16.raw","-otemp.raw","-n4"};
    /* Parse the parameters, initialize */  
    initialize(argc, argv,near,imgcol,imgrow);
    ...
    for (n_s=0;n_s<number_of_scans;n_s++) {  
        ...
        if (n_s==0) {  
            ...
            /* Print out parameters */  
            if (verbose)  
                printf("Parameters: T1=%d T2=%d T3=%d RESET=%d limit=%d\n",T1, T2, T3,RESET,limit);  
...
/***********************************************************************/  
    ...
    if (need_table)  
        printf( "Used the mapping table from file : %s\n",mappingtablefilename);  
  
    if (need_restart)  
        printf("Used restart markers with restart interval : %i\n", restart_interval);  
  
    if ( verbose )  
        printf("Marker segment bytes: %ld\n",all_header);  
  
   // printf("Total bits out: %ld  Symbols in: %ld  %5.3lf bps  (%5.3lf : 1)\n",
//               tot_out,
//               tot_in,tot_out/(double)tot_in,
//               (log((double)alpha)/log(2.0))*tot_in/tot_out);
    printf("Time = %1.3lf secs : %1.0lf KSymbols/sec\n",t1-t0,
                    (tot_in)/(1024*(t1-t0)));  
...
int read_header_6(FILE *fin, int *widthp, int *heightp, int *maxvalp, int *comp,int imgcol,int imgrow)
{  
